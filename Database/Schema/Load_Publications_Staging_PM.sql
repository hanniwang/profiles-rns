...
/****** Object:  StoredProcedure [dbo].[Load_Publications_Staging_PM]    Script Date: 05/20/2014 11:00:24 ******/
SET ANSI_NULLS ON
GO
...
CREATE procedure [dbo].[Load_Publications_Staging_PM] as
BEGIN

	truncate table [ProfilesStaging].[dbo].[PM_Load_Publications];
	insert into [ProfilesStaging].[dbo].[PM_Load_Publications]
	select	PRF_PUBS_AUTH.PubID, 
			PRF_PERSON.PersonID, 
			PRF_PUBS.PMID, --Add PMIDs
	...
	inner join [ProfilesRNS].[Profile.Data].[Publication.PubMed.General] PRF_PUBS on [ProfilesRNS].dbo.RemoveSpecialChars(FF_PUB.txt2)=[ProfilesRNS].dbo.RemoveSpecialChars(PRF_PUBS.ArticleTitle)
	inner join [ProfilesRNS].[Profile.Data].[Publication.Person.Include] PRF_PUBS_AUTH on PRF_PUBS.PMID=PRF_PUBS_AUTH.PMID
	where	ISNUMERIC(FF_PUB.expr1)=1 and ISNUMERIC(FF_PUB.sapid)=1
	...
	--------------------------------------------------------------------------------------------
		...
		From [ProfilesStaging].[dbo].[PM_Load_Publications] A
		...
		BEGIN
			...
			from [ProfilesStaging].[dbo].[PM_Load_Publications]) delete From cleanup where rownumber > 1;
		END;
