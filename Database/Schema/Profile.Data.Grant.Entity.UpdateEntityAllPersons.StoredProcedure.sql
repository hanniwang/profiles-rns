USE [ProfilesRNS]
GO
/****** Object:  StoredProcedure [Grant.Entity.UpdateEntityAllPersons.StoredProcedure.sql]    Script Date: 12/16/2013 15:16:07 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [Profile.Data].[Grant.Entity.UpdateEntityAllPersons.StoredProcedure.sql]
AS
BEGIN
  ...
  INSERT INTO #sql (s)
    ...
    FROM (
      SELECT *, '''SELECT CAST(GI.GrantID AS VARCHAR(50)) InternalIdIn  FROM [Profile.Data].[Grant.Information] as GI
        ...
        ON GI.GrantID = GA.GrantID ''' InternalIdIn
        FROM [Ontology.].DataMap
        WHERE class = 'http://vivoweb.org/ontology/core#Grant'
          AND NetworkProperty IS NULL
          AND Property IS NULL
      UNION ALL
      SELECT *, '''SELECT CAST(GA.PersonID AS VARCHAR(50))  InternalIdIn FROM [Profile.Data].[Grant.Information] as GI
        INNER JOIN [Profile.Data].[Grant.AffiliatedPeople] as GA
